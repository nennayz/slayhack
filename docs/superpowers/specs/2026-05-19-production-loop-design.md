...
**Resume compatibility for pre-SP-3 jobs:** If `python main.py --resume <job_id>` is run after SP-3 deploys for a job that was started before SP-3 (i.e., Mia/Zoe are in its message history), Robin will not have `run_mia`/`run_zoe` in its tool list and `_dispatch` will return `{"error": "Unknown tool: run_mia"}`, causing the job to fail. Mitigation: let all in-flight jobs complete before deploying SP-3. No code fix required — this is an acceptable one-time migration boundary.
...
| Resume compatibility for pre-SP-3 jobs | Acceptable known limitation — see above |
