# Age Like Fine Wine - Delivery Email Draft

Status: review_ready
Boundary: Delivery link, support terms, and refund terms are drafted but inactive until Captain approval.

## Subject Options

1. Your Fine Wine glow-up guide is ready
2. Here is Age Like Fine Wine
3. Welcome to your next glow-up chapter

## Email Body

Hi bestie,

Your copy of Age Like Fine Wine is ready.

Download placeholder:

[DOWNLOAD AGE LIKE FINE WINE]

This guide is built for women 35-44 who want to look more polished, feel more intentional, and treat aging as refinement instead of decline.

Start here:

1. Read the Fine Wine Manifesto.
2. Skim the 30-day glow-up checklist.
3. Pick one routine upgrade for this week.
4. Come back to the foundation and eye-area sections before your next makeup reset.

## Next Step CTA

If you added Fine Wine 7-Day Glow Routine, start Day 1 today. If not, use the 30-day checklist inside the guide as your first action path.

## Support Placeholder

Questions or download issues:

[APPROVED SUPPORT EMAIL]

Support response target: within 2 business days.

## Policy Note

If your file cannot be accessed and support cannot resolve the issue, the purchase can be refunded under the approved checkout terms. Duplicate purchases can be refunded. General change-of-mind refunds for a digital download are not active unless Nayz approves that policy before checkout activation.

Age Like Fine Wine provides general beauty education and routine planning. It is not medical, dermatology, supplement, or diagnosis advice. Results vary by person.

## Closing

You are not trying to look younger. You are learning how to look more like the version of you that finally knows what works.
